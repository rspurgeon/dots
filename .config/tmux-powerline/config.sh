...
# General {
	...
	# The theme to use.
	export TMUX_POWERLINE_THEME="rick"
	# Overlay directory to look for themes. There you can put your own themes outside the repo. Fallback will still be the "themes" directory in the repo.
	export TMUX_POWERLINE_DIR_USER_THEMES="${XDG_CONFIG_HOME:-$HOME/.config}/tmux-powerline/themes"
	# Overlay directory to look for segments. There you can put your own segments outside the repo. Fallback will still be the "segments" directory in the repo.
	export TMUX_POWERLINE_DIR_USER_SEGMENTS="${XDG_CONFIG_HOME:-$HOME/.config}/tmux-powerline-segments"

	# The initial visibility of the status bar. Can be {"on", "off", "2"}. 2 creates separate rows for windows and status segments.
	export TMUX_POWERLINE_STATUS_VISIBILITY="2"
	# Keep the AI status on the upper row, with windows and right-side status on the lower row.
	export TMUX_POWERLINE_WINDOW_STATUS_LINE=1
	export TMUX_POWERLINE_STATUS_FORMAT_WINDOW="${TMUX_POWERLINE_STATUS_FORMAT_WINDOW_DEFAULT}${TMUX_POWERLINE_STATUS_FORMAT_RIGHT_DEFAULT}"
	export TMUX_POWERLINE_STATUS_FORMAT_RIGHT="#[default]"
	...
	# Keep the unused status-line background aligned with the active terminal theme.
	export TMUX_POWERLINE_STATUS_STYLE="fg=#B7BDB5,bg=#000F06"
...
# macos_notification_count.sh {
	...
	# sqlite3 -list "/var/folders/g8/rqg5408d1dnfbscll8w4rj5c0000gn/0//com.apple.notificationcenter/db/db" 'select * from app_info'
...
# mailcount.sh {
	...
	export TMUX_POWERLINE_SEG_MAILCOUNT_MAILDIR_INBOX="/Users/rick.spurgeon@konghq.com/.mail/inbox/new"
	...
	export TMUX_POWERLINE_SEG_MAILCOUNT_MAILCHECKRC="/Users/rick.spurgeon@konghq.com/.mailcheckrc"
# }
